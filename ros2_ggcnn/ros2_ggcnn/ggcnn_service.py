...
class GGCNNservice(Node):
    ...
    def tf_transformation(self,from_frame,to_frame):
        ...
        self.transformation: TransformStamped = self.tf_buffer.lookup_transform(to_frame, from_frame, self.time)
    ...
    def grasp_prediction_callback(self,request,response):
        ...
        # Tranformation matrix from camera to world frame
        camera_pose = self.tf_transformation('d435_depth_frame','world')
        camera_p = np.array([camera_pose.transform.translation.x, camera_pose.transform.translation.y, camera_pose.transform.translation.z])
        camera_rot = R.as_matrix(R.from_quat([
        ...
        pos = np.dot(camera_rot, np.stack((x_cam, y_cam, depth_crop.flatten()))).T + np.array([[camera_p.x, camera_p.y, camera_p.z]])
