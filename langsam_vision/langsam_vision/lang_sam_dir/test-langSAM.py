...
file = "NP1_42"
model = LangSAM()
rgb_image_pil = Image.open(f"C:/Users/ssaserkar/Downloads/Dataset/{file}.jpg").convert("RGB")
rgb_image_height, rgb_image_width = rgb_image_pil.size
depth_image = f"C:/Users/ssaserkar/Downloads/Dataset/{file}.h5"
with h5py.File(depth_image, "r") as f:
    depth_image_np = f["depth"][:]
    depth_height, depth_width = depth_image_np.shape
text_prompt = "rubik's cube"
masks, boxes, phrases, logits = model.predict(rgb_image_pil, text_prompt)
print("Boxes:", boxes)
import numpy as np
...
scale_x = depth_width / rgb_image_width
scale_y = depth_height / rgb_image_height
...
# Example usage
# image_pil = ...  # Your PIL image
# masks = ...       # List of masks
# boxes = ...       # List of boxes [x_min, y_min, x_max, y_max]
# phrases = ...     # List of phrases associated with objects
# logits = ...      # List of logits for each class
# with h5py.File(depth_image, 'r') as f:
#     depth_image_np = f['depth'][()]
#     print(f"Depth value range: {depth_image_np.min()} - {depth_image_np.max()}")
#     visualize_depth_map(depth_image_np)

# Visualize the output
# visualize_output(rgb_image_pil, masks, boxes, phrases, logits)
cropped_rgb_images, cropped_depth_images, crop_coordinates = crop_images(rgb_image_pil, depth_image , boxes, scale_x, scale_y)

# After the calculation of crop coordinates in the crop_images function

# Print crop coordinates
print("Crop Coordinates:")
for i, (x_min, y_min, x_max, y_max) in enumerate(crop_coordinates):
    print(f"Box {i+1}: RGB - ({x_min}, {y_min}) to ({x_max}, {y_max})")

# # Visualize the cropped regions on both RGB and depth images
# for i, (rgb_crop, depth_crop) in enumerate(zip(cropped_rgb_images, cropped_depth_images)):
#     plt.figure(figsize=(10, 5))
#     plt.subplot(1, 2, 1)
#     plt.imshow(rgb_crop)
#     plt.title(f"Cropped RGB Image {i+1}")
#     plt.axis('off')

#     plt.subplot(1, 2, 2)
#     plt.imshow(depth_crop, cmap='viridis', vmin=0, vmax=1)
#     plt.title(f"Cropped Depth Image {i+1}")
#     plt.axis('off')

#     plt.show()

visualize_output_depth(cropped_rgb_images, cropped_depth_images, rgb_image_pil, boxes)

for i, depth_map in enumerate(cropped_depth_images):
    visualize_depth_map(depth_map, f"depth_map_{i}.png")

# Save the cropped depth maps as HDF5 files
for i, depth_map in enumerate(cropped_depth_images):
    save_depth_map(depth_map, f"depth_map_{i}.h5")
